...
| `--param-file`               | TEXT  | Path to `training_params.yaml` used to define the hyperparameter grid for `model_name`                     | `utils/training_params.yaml`     |
| `--training-seq-orders-file` | TEXT  | Path to `seq_orders.yaml` defining segment order/motifs for the model                             | `utils/seq_orders.yaml` |
...
# Example `training_params.yaml` Template
...
Additional models can be added to the `training_params.yaml` file by adding additional columns to the existing file. The

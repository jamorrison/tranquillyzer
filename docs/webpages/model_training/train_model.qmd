...
For a given `model_name`, `train-model` reads a parameter table (`training_params.yaml`), enumerates the specified
...
`tranquillyzer` uses a tabular parameter grid specified in `training_params.yaml`, where each column corresponds to a

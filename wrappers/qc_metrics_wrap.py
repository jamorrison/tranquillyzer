...
def _plot_read_length_per_cell(valid_path, vcols):
    ...
    df = _scan_cols(valid_path, [cell_col, "read_length"])
    if df.is_empty():
        return None

    cell_utf8 = pl.col(cell_col).cast(pl.Utf8)

    # Compute Q1 / median / Q3 per cell_id
    stats = (
        df.with_columns(cell_utf8.alias("_cid"))
        .group_by("_cid")
        .agg([
    ...
    # Sort: numeric cell-ids first (ascending), then "ambiguous"
    ...
    rows = stats.to_dicts()
    rows.sort(key=lambda r: _sort_key(r["_cid"]))
    ...
    fig.update_layout(
        xaxis=dict(
            title=f"Cell ID  ({n_cells:,} demuxed + ambiguous)",
            tickfont_size=11,
    ...
    caption = (
        ...
        "Cell IDs are sorted numerically."
    )
...
def qc_metrics_wrap(
    ...
    barcode_types = _detect_barcode_types(list(vcols))  # noqa: F841 (used by future plots)
